...
fn new_game_menu(extra_size: Option<GridSize>) -> gio::Menu {
        ...
        && !size.is_predefined()
    {
        menu.append(
            Some(&pgettext(
                concat!(
                    "on main window, entry of the menu ",
...
async fn congratulate(parent: &gtk::Widget, target_value: u64) -> bool {
    let dialog = adw::AlertDialog::new(
        ...
        Some(
            &pgettext(
                concat!(
                    "text of the dialog that appears when the user obtains ",
                    "the first 2048 tile in the game; ",
                    "the %u is replaced by the number the user wanted to reach ",
                    "(usually, 2048)"
                ),
        ...
        (
            ...
            &pgettext(
                concat!(
                    ...
                    "that appears (with default settings) when you reach 2048 ",
        ...
        (
            ...
            &pgettext(
                concat!(
                    "button in the \"Congratulations\" dialog ",
                    "that appears (with default settings) when you reach 2048; ",
                    "the player can continue playing after reaching 2048 ",
                    "(with a mnemonic that appears pressing Alt)"
                ),
                "_Keep Playing",
            ),
